...
def upgrade():
    ...
    if "user_memberships" not in tables:
        op.create_table(
            "user_memberships",
            sa.Column("id", sa.Integer(), primary_key=True, nullable=False),
    ...
    # Seed default plans only if the table is empty (fresh create_all() runs may already
    # have the table but never the row data, and this must stay idempotent either way).
    existing_plan_count = bind.execute(sa.text("SELECT COUNT(*) FROM membership_plans")).scalar()
...
def downgrade():
    bind = op.get_bind()
    tables = set(sa.inspect(bind).get_table_names())
    if "membership_orders" in tables:
        op.drop_table("membership_orders")
    if "user_memberships" in tables:
