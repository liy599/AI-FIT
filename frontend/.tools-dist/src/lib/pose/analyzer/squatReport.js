...
function summarizeSquatCoreCorrections(feedback, tempoCheck) {
    ...
    function levelFromRatio(r) {
        ...
        if (r <= 0.12)
            return { level: r === 0 ? 'ok' : 'minor', score: r === 0 ? 0 : 1 };
        if (r <= 0.35)
...
function computeSquatExplainableScore(input) {
    ...
    const assessedReps = feedback ? (feedback.session.correctReps ?? 0) + (feedback.session.incorrectReps ?? 0) : 0;
    if (!feedback)
        return { value: null, reason: 'no_feedback', breakdown: [] };
    if (input.avgTrackingQuality < 0.45)
        return { value: null, reason: 'tracking_quality_low', breakdown: [] };
    if (input.gatedFramePct >= 0.6)
        return { value: null, reason: 'too_many_gated_frames', breakdown: [] };
    if (assessedReps < 2)
        return { value: null, reason: 'not_enough_assessed_reps', breakdown: [] };
    const depthCount = feedback.session.depthInsufficientCount ?? 0;
    const kneeValgusCount = feedback.session.kneeValgusCount ?? 0;
    const heelLiftCount = feedback.session.heelLiftCount ?? 0;
    const torsoLeanCount = feedback.session.torsoLeanCount ?? feedback.session.forwardLeanCount ?? 0;
