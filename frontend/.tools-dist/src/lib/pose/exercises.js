...
export function getPoseExerciseBySlug(slug) {
    if (slug === 'lateral-raise')
        return POSE_EXERCISES['lateral-raise'];
    if (slug === 'pushup')
        return POSE_EXERCISES.pushup;
    if (slug === 'pullup')
        return POSE_EXERCISES.pullup;
    if (slug === 'bench-press')
        return POSE_EXERCISES['bench-press'];
    return POSE_EXERCISES.squat;
}
export function getPoseExerciseByType(exerciseType) {
    if (exerciseType === 'lateral_raise')
        return POSE_EXERCISES['lateral-raise'];
    if (exerciseType === 'pushup')
        return POSE_EXERCISES.pushup;
    if (exerciseType === 'pullup' || exerciseType === 'pull_up')
        return POSE_EXERCISES.pullup;
    if (exerciseType === 'bench_press' || exerciseType === 'benchpress')
        return POSE_EXERCISES['bench-press'];
    return POSE_EXERCISES.squat;
}
export function buildPoseGuidePath(slug) {
    return `/tools/pose/${slug}`;
}
export function buildPoseToolPath(slug) {
    return `/tools/pose/${slug}/tool`;
}
export function buildPoseHistoryPath(slug) {
    return `/tools/pose/${slug}/tool/history`;
}
export function buildPoseReportPath(slug, sessionId) {
