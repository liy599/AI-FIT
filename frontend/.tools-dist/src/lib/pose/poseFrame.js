...
export function buildPoseFrame(landmarks, tsMs, ctx) {
    ...
    const visible = (p) => (p && (p.visibility ?? 0) >= 0.15 ? p : undefined);
    ...
    const hasCenters = typeof centerX === 'number' &&
        typeof centerY === 'number' &&
        typeof prev?.metrics.centerX === 'number' &&
        typeof prev?.metrics.centerY === 'number';
    const dtSec = dtMs > 0 ? dtMs / 1000 : 0;
    const speedX = hasCenters && dtSec > 0 ? (centerX - prev?.metrics.centerX) / dtSec : null;
    const speedY = hasCenters && dtSec > 0 ? (centerY - prev?.metrics.centerY) / dtSec : null;
...
function computeStabilityScore(window, current) {
    const xs = [];
    const ys = [];
