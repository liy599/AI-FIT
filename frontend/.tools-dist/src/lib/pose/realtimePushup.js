export class RealtimePushupAnalyzer {
    ...
    analyze(landmarks) {
        ...
        const hipAngle = this.angleDeg(shoulder, hip, knee);
    ...
    chooseSide(landmarks) {
        ...
        for (const i of [11, 13, 15, 23, 25, 27])
            leftVis += landmarks[i]?.visibility ?? 0;
        for (const i of [12, 14, 16, 24, 26, 28])
            rightVis += landmarks[i]?.visibility ?? 0;
    ...
    sideAlignmentDeg(shoulder, otherShoulder) {
