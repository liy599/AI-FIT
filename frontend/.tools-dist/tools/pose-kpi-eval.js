...
import { runOfflineCase } from './pose-offline-core';
...
function validateManifest(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        fail('Invalid manifest JSON: expected object');
    const obj = raw;
    if (obj.schemaVersion !== 1)
        fail('Invalid manifest JSON: schemaVersion must be 1');
    if (!Array.isArray(obj.cases))
        fail('Invalid manifest JSON: cases must be array');
    return obj;
}
function validateCase(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        fail('Invalid case JSON: expected object');
    const obj = raw;
    if (obj.schemaVersion !== 1)
        fail('Invalid case JSON: schemaVersion must be 1');
    if (typeof obj.id !== 'string' || !obj.id.trim())
        fail('Invalid case JSON: id required');
    if (!obj.exercise || typeof obj.exercise !== 'object' || Array.isArray(obj.exercise))
        fail('Invalid case JSON: exercise required');
    if (!Array.isArray(obj.frames))
        fail('Invalid case JSON: frames must be array');
    return obj;
}
function readLabels(caseJson) {
...
function getNumberFromReport(report, dottedPath) {
...
function getPredictedReps(report) {
    ...
    if (detailsRep !== null)
        return Math.round(detailsRep);
    return null;
}
function getPredictedUnassessedReps(report) {
    const n = getNumberFromReport(report, 'keyMetrics.unassessedReps');
    if (n !== null)
        return Math.round(n);
    return null;
}
function extractCoreCorrectionLevels(report) {
...
function predictedIssuePresence(report) {
...
function addConfusion(cur, pred, gt) {
    if (pred && gt)
        cur.tp += 1;
    else if (pred && !gt)
        cur.fp += 1;
...
async function main() {
    ...
    if (!manifestArg)
        fail('Usage: npm run pose:kpi -- --manifest <manifest.json> [--out <report.json>]');
    ...
    const rawManifest = await readJsonFile(absManifestPath);
    ...
    const confusions = Object.fromEntries(correctionTypes.map((t) => [t, { tp: 0, fp: 0, tn: 0, fn: 0 }]));
    const unassessedRates = [];
    const skipped = [];
    const unstable = [];
    for (const item of manifest.cases) {
        const casePath = path.resolve(manifestDir, item.caseFile);
        const rawCase = await readJsonFile(casePath);
        ...
        if (repsA !== repsB)
            unstable.push({ id: item.id, reason: `rep_count_unstable:${repsA ?? 'null'}!=${repsB ?? 'null'}` });
        ...
        for (const t of correctionTypes) {
            if (predIssuesA[t] !== predIssuesB[t])
                unstable.push({ id: item.id, reason: `issue_unstable:${t}` });
        }
        const gtReps = typeof labels.reps === 'number' ? labels.reps : typeof labels.totalReps === 'number' ? labels.totalReps : null;
        if (gtReps !== null && repsA !== null)
            repErrors.push(Math.abs(repsA - gtReps));
    ...
    const metricsByIssue = Object.fromEntries(correctionTypes.map((t) => {
    ...
    if (repMae !== null)
        process.stdout.write(`- rep MAE: ${Math.round(repMae * 1000) / 1000}\n`);
    if (avgUnassessedRate !== null)
        process.stdout.write(`- avg unassessed rate: ${Math.round(avgUnassessedRate * 1000) / 10}%\n`);
    for (const t of correctionTypes) {
        const item = result.perIssue[t];
    ...
    if (unstable.length > 0)
        process.stdout.write(`- warnings: ${unstable.length} unstable output(s)\n`);
    if (outArg) {
