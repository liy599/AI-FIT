...
function normalizeReportForSnapshot(report, ignorePaths) {
    const clone = structuredClone(report);
    for (const p of ignorePaths)
        deletePath(clone, p);
    return clone;
}
function diffWithTolerance(expected, actual, tol, basePath, max, diffs) {
    if (diffs.length >= max)
        return;
    if (typeof expected === 'number' && typeof actual === 'number') {
        ...
        if (Number.isFinite(e) && Number.isFinite(a)) {
            if (Math.abs(e - a) > tol)
                diffs.push({ path: basePath, expected: e, actual: a });
            return;
        }
    }
    if (expected === actual)
    ...
    const eObj = expected && typeof expected === 'object' && !Array.isArray(expected) ? expected : null;
    const aObj = actual && typeof actual === 'object' && !Array.isArray(actual) ? actual : null;
    if (eObj && aObj) {
...
async function main() {
    ...
    for (const item of manifest.cases) {
        ...
        let expected = null;
        try {
            expected = await readJsonFile(snapshotPath);
        }
        catch {
            failures.push({
                id: item.id,
