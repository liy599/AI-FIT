...
export class RealtimeLateralRaiseAnalyzer {
  ...
  private detectState(armRaise: number | null): 's1' | 's2' | 's3' | null {
    if (armRaise === null) return null
    if (armRaise >= 68) return 's3'
    if (armRaise >= 28) return 's2'
    return 's1'
  }
  ...
  private shoulderRaiseDeg(
    shoulder?: NormalizedLandmark | null,
    elbow?: NormalizedLandmark | null,
    hip?: NormalizedLandmark | null
  ): number | null {
