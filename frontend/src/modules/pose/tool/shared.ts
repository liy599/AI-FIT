...
export const POSE_TOOL_MESSAGES = {
  ...
  squatTuningUnsupported: 'Skip: only squat supports backend tuning defaults.',
  squatTuningSaved: 'Saved to backend defaults. It will persist across refresh and restart.',
  squatTuningSaveFailed: 'Failed to save backend defaults.'
} as const
...
const TEACHING_COPY_BY_SLUG: Record<string, PoseTeachingCopy> = {
  pushup: {
    cameraAngle: 'Place your camera at a true side view (about 90 deg) so your whole body stays in frame.',
    tipsLines: [
      'Start in a straight line from head to heels.',
      'Hands under shoulders, core tight.',
      'Lower with control until elbows reach about 90 deg.',
      'Keep elbows slightly tucked (not flared).',
      'Press up by pushing the floor away; avoid hips sagging or piking; keep neck neutral.'
    ]
  },
  'bent-over-row': {
    cameraAngle: 'Use a side view (about 60 deg-90 deg) to clearly see your hip hinge and dumbbell path.',
    tipsLines: [
  ...
  'lateral-raise': {
    cameraAngle: 'Set the camera directly in front (0 deg) so both arms are equally visible.',
    tipsLines: [
  ...
  squat: {
    cameraAngle: 'Use a 30 deg-45 deg front angle (or a true side view at 90 deg) to capture hips, knees, and ankles clearly.',
    tipsLines: [
