...
import { resolveAnalyzerTuning, type PoseRuntimeRules } from '../helpers'
import type { OfflineOverlayTone, OfflineProgress, OfflineReplayData } from './types'
import { buildOfflineArchivePayload, resolveOfflineArchiveStatusMessage, validateOfflineArchiveUser } from './offlineArchive'
import {
  buildOfflinePoseReport,
  extractOfflinePoseFromLocalVideo,
  resolveOfflineViewAngle
} from './offlineAnalysisPipeline'
import { buildOfflineOverlayFrames } from './replay'
  ...
  return useCallback(async () => {
    ...
    try {
      ...
      const overlayFrames = buildOfflineOverlayFrames({
        ...
        analyzerTuning: resolveAnalyzerTuning(exercise.slug, poseRuntimeRules, squatTuningForVideo),
