...
function buildBenchPressReplaySuggestions(feedback: RealtimeFeedback | null, sortedIssues: Array<[string, number]>, fallbackSuggestion: string) {
  ...
  if (feedback) {
    if ((feedback.session.totalReps ?? 0) <= 0) {
      suggestions.add('Lower until your elbows reach a stable depth, pause briefly, then press to a controlled lockout.')
    }
    if ((feedback.trackingQuality ?? 0) < 0.5) {
      suggestions.add('Improve lighting and keep shoulders, elbows, wrists, and torso fully visible throughout the set.')
    }
  }
