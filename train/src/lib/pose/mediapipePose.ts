...
export type PoseExtractOptions = {
  maxFrames?: number
  targetFps?: number
...
export async function extractPose33FromVideoUrl(videoUrl: string, opts: PoseExtractOptions = {}): Promise<{ fps: number; frames: PoseFrame[] }> {
  const { maxFrames = 4000, targetFps = 30, minVisibility = 0.2, onProgress } = opts

  if (typeof window === 'undefined') throw new Error('Browser only')

  const [{ FilesetResolver, PoseLandmarker }] = await Promise.all([import('@mediapipe/tasks-vision')])
  ...
  for (let i = 0; i < total; i++) {
    ...
    frames.push({
      tMs: t,
      landmarks: lm ? filterByVisibility(lm as NormalizedLandmark[], minVisibility) : null
    })
...
function filterByVisibility(lms: NormalizedLandmark[], minVisibility: number): NormalizedLandmark[] {
  return lms.map((p) => {
    ...
    if (v < minVisibility) return { x: p.x, y: p.y, z: p.z, visibility: v, presence: p.presence }
    return p
  })
}
