...
export async function createMoveNetDetector(opts?: { variant?: 'lightning' | 'thunder'; enableSmoothing?: boolean }): Promise<MoveNetDetector> {
  ...
  const variant = opts?.variant ?? 'thunder'
  const enableSmoothing = opts?.enableSmoothing ?? true
  const detector = await poseDetection.createDetector(poseDetection.SupportedModels.MoveNet, {
    modelType: variant === 'lightning' ? poseDetection.movenet.modelType.SINGLEPOSE_LIGHTNING : poseDetection.movenet.modelType.SINGLEPOSE_THUNDER,
    enableSmoothing
  })
