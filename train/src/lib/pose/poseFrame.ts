...
export function buildPoseFrame(
  ...
  const v = (p: NormalizedLandmark | undefined) => (p && (p.visibility ?? 0) >= 0.15 ? p : undefined)
  const lShoulderV = v(lShoulder)
  const rShoulderV = v(rShoulder)
  const lHipV = v(lHip)
  const rHipV = v(rHip)
  const lKneeV = v(lKnee)
  const rKneeV = v(rKnee)
  const lAnkleV = v(lAnkle)
  const rAnkleV = v(rAnkle)
  ...
  const hasCenters = typeof centerX === 'number' && typeof centerY === 'number' && typeof prev?.metrics.centerX === 'number' && typeof prev?.metrics.centerY === 'number'
  ...
  const winWithCurrent = [...window]
  const jitterScore = computeStabilityScore(winWithCurrent, { tsMs, centerX, centerY })
  ...
  return {
    ...
    metrics: {
      ...
      speedX,
      speedY,
      stabilityScore: jitterScore
    }
  }
}

function computeStabilityScore(window: PoseFrame[], current: { tsMs: number; centerX: number | null; centerY: number | null }) {
  const xs: number[] = []
  const ys: number[] = []
  for (const f of window) {
    const x = f.metrics.centerX
    const y = f.metrics.centerY
